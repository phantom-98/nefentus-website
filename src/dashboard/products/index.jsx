...
const ProductBody = () => {
  ...
  async function loadProducts() {
    ...
    const newSignedImagePaths = await Promise.all(
      newProducts.map((product) => dashboardApi.getSignedImagePath(product.id)),
    );
    if (newSignedImagePaths) setSignedImagePaths(newSignedImagePaths);
  }
  ...
  return (
    <>
      <div className={`dashboard-body`}>
        ...
        >
          <Button color="white" onClick={() => showModal(null)}>
            Create new product
          </Button>
        </TopInfo>
        ...
        <div className={styles.row}>
          {products.map((product, index) => (
            <Card
              ...
              onClickEdit={() => showModal(product.id)}
